...
<div class="container-fluid flex-grow-1 container-p-y">
    ...
    <form action="{{ route('admin.quotations.update', $quotation->id) }}" method="POST" id="quotationForm">
        ...
        <div class="q-edit-header">
            <h4><i class="bx bx-edit-alt"></i> Edit Quotation #{{ $quotation->quotation_number }}</h4>
        ...
        <div class="card q-card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                @php
                    $isMrpActive = old('show_mrp', isset($quotation->show_mrp) ? (bool)$quotation->show_mrp : true);
                @endphp
                <div class="d-flex align-items-center gap-2 bg-light px-3 py-1 rounded-3 border" style="border-color: #e8e5f0 !important;">
                    ...
                    <div class="form-check form-switch mb-0 d-flex align-items-center gap-1">
                        <input class="form-check-input" type="checkbox" name="show_mrp" id="show_mrp" value="1" {{ $isMrpActive ? 'checked' : '' }} style="cursor: pointer; width: 2.2em; height: 1.15em;">
                        <span class="badge {{ $isMrpActive ? 'bg-success' : 'bg-secondary' }}" id="mrpBadge" style="font-size: 0.72rem; letter-spacing: 0.5px;">{{ $isMrpActive ? 'SHOW MRP' : 'NO MRP' }}</span>
                    </div>
                </div>
            </div>
            <div class="card-body">
                <div class="row g-3">
                    ...
                    <div class="col-md-2">
                        ...
                        <input type="text" name="quotation_number" class="form-control" value="{{ $quotation->quotation_number }}" readonly style="background: #f8f7ff;">
                    </div>
                    <div class="col-md-2">
                        <label class="form-label">Date <span class="text-danger">*</span></label>
                        <input type="date" name="date" class="form-control @error('date') is-invalid @enderror" value="{{ old('date', $quotation->date ? date('Y-m-d', strtotime($quotation->date)) : date('Y-m-d')) }}" required>
                        @error('date') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-2">
                        <label class="form-label">Valid Until</label>
                        <input type="date" name="valid_until" class="form-control @error('valid_until') is-invalid @enderror" value="{{ old('valid_until', $quotation->valid_until ? date('Y-m-d', strtotime($quotation->valid_until)) : '') }}">
                ...
                <div class="row">
                    <div class="col-lg-6 mb-3 mb-lg-0">
                        <div class="mb-3">
                            ...
                            <textarea name="notes" class="form-control @error('notes') is-invalid @enderror" rows="3" placeholder="Add any notes...">{{ old('notes', $quotation->notes) }}</textarea>
                            @error('notes') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="mb-3">
                            <label class="notes-label"><i class="bx bx-file"></i> Terms & Conditions</label>
                            <textarea name="terms_conditions" class="form-control @error('terms_conditions') is-invalid @enderror" rows="3" placeholder="Add terms & conditions...">{{ old('terms_conditions', $quotation->terms_conditions) }}</textarea>
