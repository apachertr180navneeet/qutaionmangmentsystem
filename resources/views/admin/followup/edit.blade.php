...
<div class="container-fluid flex-grow-1 container-p-y">
    <h5 class="py-2 mb-2">
        <span class="text-primary fw-light">Edit Follow-up</span>
    </h5>
    <div class="card">
        <div class="card-body">
            <form action="{{ route('admin.followups.update', $followUp->id) }}" method="POST">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <select name="quotation_id" class="form-select select2 @error('quotation_id') is-invalid @enderror" required>
                            ...
                            @foreach($quotations as $quotation)
                                <option value="{{ $quotation->id }}" {{ old('quotation_id', $followUp->quotation_id) == $quotation->id ? 'selected' : '' }}>{{ $quotation->quotation_number }} - {{ $quotation->customer->company_name ?? '' }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="date" name="follow_up_date" class="form-control @error('follow_up_date') is-invalid @enderror" value="{{ old('follow_up_date', $followUp->follow_up_date) }}" required>
                        @error('follow_up_date') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Follow-up Time</label>
                        <input type="time" name="follow_up_time" class="form-control @error('follow_up_time') is-invalid @enderror" value="{{ old('follow_up_time', $followUp->follow_up_time) }}">
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea name="notes" class="form-control @error('notes') is-invalid @enderror" rows="3">{{ old('notes', $followUp->notes) }}</textarea>
                        @error('notes') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                </div>
                <div class="pt-4">
                    <button type="submit" class="btn btn-primary"><i class="bx bx-save"></i> Update Follow-up</button>
